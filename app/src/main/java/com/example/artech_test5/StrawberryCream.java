...
public class StrawberryCream extends Fragment implements ArduinoListener{
    private Arduino arduino;
    private FragmentManager fragmentManager;
    private FragmentTransaction transaction;
    private Gif gif;
    private String value = "60,0,0";
    private int count = 11;

    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ViewGroup rootView = (ViewGroup)inflater.inflate(R.layout.strawberrycream_fragment, container, false);
        arduino = new Arduino(getActivity().getApplicationContext());
        strawberryCreamtHandler(rootView);
        return rootView;
    }

    private void strawberryCreamtHandler(ViewGroup rootView) {

        ImageView teaSelect = (ImageView)rootView.findViewById(R.id.teaSelect15);
        ...
        Fragment myFragment = fragmentManager.findFragmentByTag("strawberryCream");
        transaction = fragmentManager.beginTransaction();
        home.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ((MainActivity)getActivity()).black.setVisibility(View.INVISIBLE);
                transaction.remove(StrawberryCream.this).commitAllowingStateLoss();
