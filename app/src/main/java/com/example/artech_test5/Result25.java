...
public class Result25 extends Fragment  {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ViewGroup rootView = (ViewGroup) inflater.inflate(R.layout.result25_fragment, container, false);
        result25ImgHandler(rootView);
        return rootView;
    }
    private void result25ImgHandler(ViewGroup rootView) {
        ImageView rosemary = (ImageView)rootView.findViewById(R.id.rosemary);
        ImageView back = (ImageView)rootView.findViewById(R.id.img_back43);
        ImageView home = (ImageView)rootView.findViewById(R.id.img_home43);

        fragmentManager = getActivity().getSupportFragmentManager();
        Fragment myFragment = fragmentManager.findFragmentByTag("result25");

        transaction = fragmentManager.beginTransaction();

        home.setOnClickListener(v -> {
            ((MainActivity)getActivity()).black.setVisibility(View.INVISIBLE);
            transaction.remove(Result25.this).commitAllowingStateLoss();
