...
public class Result45 extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ViewGroup rootView = (ViewGroup) inflater.inflate(R.layout.result45_fragment, container, false);
        result45ImgHandler(rootView);
        return rootView;
    }

    private void result45ImgHandler(ViewGroup rootView) {
        ImageView roseFlower = (ImageView)rootView.findViewById(R.id.roseFlower);
        ImageView back = (ImageView)rootView.findViewById(R.id.img_back68);
        ImageView home = (ImageView)rootView.findViewById(R.id.img_home68);

        fragmentManager = getActivity().getSupportFragmentManager();
        Fragment myFragment = fragmentManager.findFragmentByTag("result45");
        transaction = fragmentManager.beginTransaction();

        home.setOnClickListener(v -> {
            ((MainActivity)getActivity()).black.setVisibility(View.INVISIBLE);
            transaction.remove(Result45.this).commitAllowingStateLoss();
