...
public class Result20 extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ViewGroup rootView = (ViewGroup) inflater.inflate(R.layout.result20_fragment, container, false);
        result20ImgHandler(rootView);
        return rootView;
    }
    private void result20ImgHandler(ViewGroup rootView) {
        ImageView chamomile = (ImageView)rootView.findViewById(R.id.chamomile);
        ImageView back = (ImageView)rootView.findViewById(R.id.img_back36);
        ImageView home = (ImageView)rootView.findViewById(R.id.img_home36);

        fragmentManager = getActivity().getSupportFragmentManager();
        Fragment myFragment = fragmentManager.findFragmentByTag("result20");
        transaction = fragmentManager.beginTransaction();

        home.setOnClickListener(v -> {
            ((MainActivity)getActivity()).black.setVisibility(View.INVISIBLE);
            transaction.remove(Result20.this).commitAllowingStateLoss();
