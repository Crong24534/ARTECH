...
public class Result29 extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ViewGroup rootView = (ViewGroup) inflater.inflate(R.layout.result29_fragment, container, false);
        result29ImgHandler(rootView);
        return rootView;
    }

    private void result29ImgHandler(ViewGroup rootView) {
        ImageView rooibos = (ImageView)rootView.findViewById(R.id.rooibos);
        ImageView back = (ImageView)rootView.findViewById(R.id.img_back47);
        ImageView home = (ImageView)rootView.findViewById(R.id.img_home47);

        fragmentManager = getActivity().getSupportFragmentManager();
        Fragment myFragment = fragmentManager.findFragmentByTag("result29");
        transaction = fragmentManager.beginTransaction();

        home.setOnClickListener(v -> {
            ((MainActivity)getActivity()).black.setVisibility(View.INVISIBLE);
            transaction.remove(Result29.this).commitAllowingStateLoss();
